...
// ***************************************************************** 
function Folder(folderDescription, hreference, title) //constructor 
{ 
  //constant data 
  ...
  if ( hreference == null ) {
	this.iconSrc 		= 	ICONPATH + "building.gif";
	this.iconSrcClosed 	= 	ICONPATH + "building.gif";
  } else {
	  if ( folderDescription == "Controle de Acesso" ) {
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a class=lka href='javascript:clickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a class=lka>" 
  ...
  if (USEICONS)
  {
    ...
    docW = docW + "<img id='folderIcon" + this.id + "' name='folderIcon" + this.id + "' src='" + this.iconImageSrc() + "' border=0" + " title='" + this.title + "'></a>"
  }
  else
  {
	  if (this.prependHTML == "")
  ...
  if (WRAPTEXT)
	  docW = docW + "</td>"+this.prependHTML+"<td valign=middle " + " title='" + this.title + "' width=100%>"
  else
	  docW = docW + "</td>"+this.prependHTML+"<td valign=middle " + " title='" + this.title + "' nowrap width=100%>"
  if (USETEXTLINKS) 
  { 
...
{ 
  ...
  { 
	if (USEFRAMES)
	  docW = docW + "<a class=lka href='" + this.hreference + "' TARGET=\"basefrm\" "
	else
	  docW = docW + "<a class=lka href='" + this.hreference + "' TARGET=_top "
...
function Item(itemDescription, itemLink, target, title) // Constructor 
{ 
  // constant data 
  ...
  if (itemDescription == 'Sair') {
//	  this.iconSrc 			= 	ICONPATH + "folderLogout.gif" 
	  this.iconSrc 			= 	ICONPATH + "door_out.gif" 
  } else {
  	if (itemDescription == 'Senha') {
//	  this.iconSrc 			= 	ICONPATH + "passWord.gif" 
	  this.iconSrc 			= 	ICONPATH + "lock_edit.gif" 
	} else {
  		if (itemDescription == 'Unidade') {
	  		this.iconSrc 			= 	ICONPATH + "chart_organization.gif" 
		} else {
...
{ 
  var leftSide = leftSideHTML(this.leftSideCoded)
  var docW = ""
  var fullLink = "href=\""+this.link+"\" title=\"" + this.title + "\" target=\""+this.target+"\" onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');return false;\"";
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a class=lka " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' border=0>" + "</a>"
  else
	  if (this.prependHTML == "")
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a class=lka " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function blockStartHTML(idprefix) {
  ...
  if (browserVersion == 2) 
    ...
    docW = "<div " + idParam + " style='display:block; visibility:visible;'>"
...
function isLinked(hrefText) {
    ...
    result = (result && hrefText.indexOf('op(') < 0);
    return result;
}
...
function gFld(description, hreference, title) 
{ 
  folder = new Folder(description, hreference, title);
  return folder;
} 
 
function gLnk(optionFlags, description, linkData, title) 
{ 
  var fullLink = "";
  var targetFlag = "";
  var target = "";
  ...
  linkItem = new Item(description, protocol+linkData, target, title)
  return linkItem 
} 
...
function preLoadIcons() {
	...
	auxImg.src = ICONPATH + "folderLogout.gif";
	auxImg.src = ICONPATH + "passWord.gif";
}
