...
//	{
//		iFranquia						=	Integer.parseInt(txtFrq);
//	}
	...
	{
		lColaborador					=	UTLUtils.getStringToLong( txtUid );
	}
...
function Submeter(acao) {
	if (acao == "Ext") {
		...
		document.form1.action="/infinity";
	}
	if (acao == "Cli") {
		document.form1.txtAcao.value = "";
...
//===============================================//
	...
	if (acao == "N") {
		document.form1.txtContrato.value = "0";
...
//	alert("Submit: " + acao + "\ntxtAcao:" + document.form1.txtAcao.value + "\naction:" + document.form1.action);
...
%>
				<tr>
					<td width="100%" height="100%" align="center" valign="top">
						...
						<tr class="selecF12" height="50"><td colspan="2" valign="top">CONTRATO NOVO CONSULTA<%=txtTitulo%></td></tr>
<%
	if ( intExibir == EXIBIR_PROCURA )
	{
%>
...
//		CADColaborador _colaborador		=	new CADColaborador( CADColaborador.BUSCA_CONTRATO, iFranquia, null, 0, 0, 0, 0, conn );
		CADColaborador _colaborador		=	null;
		if ( iAcesso == CADColaborador.CADASTRO_CONSULTA || iAcesso == CADColaborador.CADASTRO_FRANQUIA )
		{
			_colaborador				=	new CADColaborador( lColaborador, conn );
			_colaborador.start();
		}
		else
		{
			_colaborador				=	new CADColaborador( CADColaborador.BUSCA_NOVOS, -1, null, 0, 0, 0, 0, conn );
		}
...
<%
		...
		if ( iAcesso == CADColaborador.CADASTRO_CONSULTA || iAcesso == CADColaborador.CADASTRO_FRANQUIA )
		{
			intSituacao					=	CADClienteNovo.SITUACAO_COODENADOR;
		}
		if ( iAcesso == CADColaborador.CADASTRO_FINANCEIRO )
		{
			intSituacao					=	CADClienteNovo.SITUACAO_FINANCEIRO;
		}
		if ( bTTot.doubleValue() == 0 && bTCms.doubleValue() == 0 )
		{
			_contrato					=	new CADContratoNovo( intSituacao, lCliente, lCoordenador, txtBusca, 0, 0, 0, 0, conn );
		...
		_contrato						=	new CADContratoNovo( intSituacao, lCliente, lCoordenador, txtBusca, UTLUtils.getStringToInt(txtOrdem), intRegistros, intPagina, intTotal, conn );
...
function OnKeyDown(e) {
    ...
    k = e.which || e.keyCode;
	if ( k == 13 ) {
		document.form1.txtContrato.value = posicaoAnterior.getAttribute("codigo");
		Submeter("M");	
	} else {
		if ( k == 40 || k == 38 ) {
